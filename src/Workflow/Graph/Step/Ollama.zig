...
pub const Image = struct {
    ...
    pub const Source = union(enum) {
        ...
        pub fn get(
            ...
            return switch (self) {
                .input => |*input| input.get(alloc, config, inputs, graph, secrets, server),
    ...
    pub fn toOllama(
        ...
        const source = try self.source.get(alloc, config, inputs, graph, secrets, server);
...
pub fn run(
    ...
    const prompt = try Workflow.format(alloc, self.prompt, config, inputs, graph, secrets, server, self.inputs);
    ...
    if (self.images) |self_images| {
        for (self_images) |self_img| {
            const img = try self_img.toOllama(alloc, config, inputs, graph, secrets, server);
